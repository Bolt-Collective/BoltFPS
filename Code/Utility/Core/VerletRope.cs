...
public class VerletRope : Component
{
	...
	public static VerletRope AddRope( GameObject point1, GameObject point2, float len, string material = "materials/default/rope01.vmat", bool rigid = false )
	{
		...
		vertletRope.Attachment = ropePoint2;

		var lineRenderer = ropePoint1.AddComponent<LineRenderer>();
		lineRenderer.Points = [ropePoint1, ropePoint2];
		lineRenderer.Width = 1f;
		lineRenderer.Color = Color.White;
		lineRenderer.Lighting = true;
		lineRenderer.CastShadows = true;
		lineRenderer.SplineInterpolation = 4;
		lineRenderer.Texturing = lineRenderer.Texturing with { Material = Material.Load( material ), WorldSpace = true, UnitsPerTexture = 32 };
		lineRenderer.Face = SceneLineObject.FaceMode.Cylinder;

		ropePoint2.Network.AssignOwnership( Connection.Host );
