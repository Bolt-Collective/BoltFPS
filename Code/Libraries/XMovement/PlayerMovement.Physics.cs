...
public partial class PlayerMovement : Component
{
	...
	[Range( 0, 200 )] [Property] public float Height { get; set; } = 72.0f;

	[Range( 0, 50 )] [Property] public float StepHeight { get; set; } = 18.0f;

	[Range( 0, 90 )] [Property] public float GroundAngle { get; set; } = 45.5734f;
	...
	[ReadOnly, Property, Sync] public Vector3 Velocity { get; set; }

	[ReadOnly, Property, Sync] public Vector3 BaseVelocity { get; set; }

	[ReadOnly, Property, Sync] public bool IsOnGround { get; set; }
	...
	/// </summary> 
	[Property]
	ModelCollider PlayerColliderModel { get; set; }
	...
	public virtual SceneTrace BuildTrace( Vector3 from, Vector3 to, float liftFeet = 0.0f, float scaleMult = 1f )
	{
		var box = BoundingBox;
		if ( liftFeet > 0 )
		{
		...
		box.Mins *= WorldScale * scaleMult;
		box.Maxs *= WorldScale * scaleMult;
	...
	public SceneTraceResult MoveTraceResult { get; set; }
	...
	public void StayOnGround()
	{
		...
		if ( tr.Fraction > 0.0f && tr.Fraction < 1.0f && !tr.StartedSolid &&
		     Vector3.GetAngle( Vector3.Up, tr.Normal ) <= GroundAngle )
		{
	...
	public virtual void ChangeGround( SceneTraceResult pm )
	{
		...
		GroundCollider = pm.Collider as Collider;
	...
	bool TryUnstuck()
	{
		...
		for ( int i = 0; i < AttemptsPerTick; i++ )
		{
			...
			if ( i <= 2 )
			{
				...
				if ( debug_playermovement_unstick )
					DebugOverlay.Box( BoundingBox, Color.Cyan, 2, Transform.World.WithRotation( Rotation.Identity ) );
			}

			// Try base velocity 
			if ( (PhysicsBodyVelocity.Length > 0 ||
			      (PhysicsBodyRigidbody.IsValid() && PhysicsBodyRigidbody.WorldPosition != WorldPosition)) && i < 80 )
			{
				...
				if ( debug_playermovement_unstick )
					DebugOverlay.Box( BoundingBox, Color.Green, 2, Transform.World.WithRotation( Rotation.Identity ) );
			...
			else if ( i < 4 )
			{
				...
				if ( debug_playermovement_unstick )
					DebugOverlay.Box( BoundingBox, Color.Yellow, 2, Transform.World.WithRotation( Rotation.Identity ) );
			}
			else
			{
				...
				if ( debug_playermovement_unstick )
					DebugOverlay.Box( BoundingBox, Color.Magenta, 2,
						Transform.World.WithRotation( Rotation.Identity ) );
			}
