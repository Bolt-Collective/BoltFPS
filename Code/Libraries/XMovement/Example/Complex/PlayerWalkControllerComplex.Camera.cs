...
{
	[Property, Group( "Camera" ), Change( "SetupCamera" ), Sync]
	...
	public void UpdateCamera()
	{
		if ( CameraMode == CameraModes.ThirdPerson )
		{
			...
			var tr = Scene.Trace.Ray( start, end ).IgnoreDynamic().WithoutTags("player").Run();
			if(tr.Hit)
				fraction = tr.Fraction - 0.1f;
