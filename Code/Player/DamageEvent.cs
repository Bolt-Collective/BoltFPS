...
/// <param name="External">Was this a direct attack</param>
	...
	bool External = false )
{
	...
	public void DisplayFeed()
	{
		...
		//I don't FUCKING know, getcomponent just returns fucking null for some reason. FUCK ME
		foreach ( var component in Victim?.Components?.GetAll()! )
		{
			if ( component.GetType() == typeof(Pawn) )
			{
				var pawn = (Pawn)component;
				victimColour = pawn.Owner?.Team.Color ?? Color.White;
			}
		}

		if ( Hitbox.HasFlag( HitboxTags.Head ) )
			killMethods.Add( "icons/kill methods/headshot.png" );

		if ( WasExplosion )
			killMethods.Add( "icons/kill methods/explosion.png" );
...
public abstract class ModifyDamageEvent : IGameEvent
{
	...
	{
		DamageInfo = DamageInfo with { Damage = 0f, ArmorDamage = 0f };
	}
