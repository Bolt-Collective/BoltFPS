...
PS
{
	...
	float4 MainPs( PixelInput i ) : SV_Target0
	{
		...
		float l_2 = pow( l_1, 3 );
		float l_3 = 1 / l_2;
		float3 l_4 = GetTangentViewVector( i.vPositionWithOffsetWs.xyz + g_vHighPrecisionLightingOffsetWs.xyz, i.vNormalWs, i.vTangentUWs, i.vTangentVWs );
		float3 l_5 = normalize( l_4 );
		float l_6 = g_flDistance;
		float3 l_7 = l_5 * float3( l_6, l_6, l_6 );
		float l_8 = l_3 - 1;
		float l_9 = l_8 * -0.5;
		float3 l_10 = l_7 + float3( l_9, l_9, l_9 );
		float2 l_11 = TileAndOffsetUv( i.vTextureCoords.xy, float2( l_3, l_3 ), l_10.xy );
		float4 l_12 = Tex2DS( g_tRetical, g_sSampler0, l_11 );
		float4 l_13 = l_0 * l_12;
		float2 l_14 = float2( 0.5, 0.5 );
		float l_15 = distance( l_11, l_14 );
		float l_16 = g_flVignetteSize;
		float l_17 = l_15 / l_16;
		float l_18 = 1 - l_17;
		float l_19 = g_flVignetteHardness;
		float l_20 = l_18 * l_19;
		float l_21 = saturate( l_20 );
		float4 l_22 = l_13 * float4( l_21, l_21, l_21, l_21 );
		float4 l_23 = Tex2DS( g_tRoughness, g_sSampler0, i.vTextureCoords.xy );
